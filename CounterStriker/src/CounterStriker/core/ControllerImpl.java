...
public class ControllerImpl implements Controller{
    ...
    public String addPlayer(String type, String username, int health, int armor, String gunName) {
        ...
        switch (type){
            case "CounterTerrorist":
                CounterTerrorist counterTerrorist = new CounterTerrorist(username,health,armor,byName);
                players.add(counterTerrorist);
                return String.format(SUCCESSFULLY_ADDED_PLAYER, counterTerrorist.getUsername());
            case "Terrorist":
                Terrorist terrorist = new Terrorist(username, health, armor, byName);
                players.add(terrorist);
                return String.format(SUCCESSFULLY_ADDED_PLAYER, terrorist.getUsername());
            default:
    ...
    public String startGame() {
        List<Player> playerList = players.getModels().stream().filter(Player::isAlive).collect(Collectors.toList());

        return field.start(playerList);
    }
